...
async def wa_sync_import(
    body: WaSyncImportRequest,
    profile: Profile = Depends(get_current_profile),
    db: Session = Depends(get_db),
):
    existing_phones = {
        row.phone for row in
        db.query(Contact.phone).filter(Contact.profile_id == profile.id).all()
    }
    created = 0
    skipped = 0
...
def bulk_tag_contacts(
    ...
    updated = (
        db.query(Contact)
        .filter(Contact.id.in_(body.contact_ids), Contact.profile_id == profile.id)
...
def delete_all_contacts(
    ...
    deleted = db.query(Contact).filter(Contact.profile_id == profile.id).delete()
    db.commit()
...
def create_contact(
    ...
    contact = Contact(**body.model_dump(), profile_id=profile.id)
    db.add(contact)
    try:
        db.commit()
        ...
        raise HTTPException(
            status_code=409,
            detail=f"A contact with phone {body.phone} already exists in this profile",
        )
    db.refresh(contact)
    return contact
...
def get_contact(
    ...
    contact = (
        ...
        .filter(Contact.id == contact_id, Contact.profile_id == profile.id)
        .first()
    )
    if not contact:
...
def update_contact(
    ...
    contact = db.query(Contact).filter(Contact.id == contact_id, Contact.profile_id == profile.id).first()
    if not contact:
        raise HTTPException(status_code=404, detail="Contact not found")
